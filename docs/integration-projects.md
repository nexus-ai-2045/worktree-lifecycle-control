...
3. `python -m worktree_lifecycle_control evidence-from-closeout` が観測時刻付き collect JSON を `integration-evidence-v3` へ正規化する。collect は `observed_at` (取得時刻) と `pr_state.mergedBy` を出力する。どちらかが無い入力は fail-closed で失敗する (`mergedBy` だけは `--actor` で補える)。公開済みの v1 / v2 schema は互換性維持のため残し、変更しない。
